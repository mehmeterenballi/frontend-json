fetch("./assets/json/product-list.json")
    ...
    .then(data => {
        ...
        let jsonObject = data.responses[0][0].params.recommendedProducts["Beyaz Eşya & Küçük Ev Aletleri > Isıtma, Soğutma Sistemi"];
        let text = "";
        ...
        for (const key in jsonObject) {
            if (Object.hasOwnProperty.call(jsonObject, key)) {
                ...
                text += '</div>'
                    ...
                    + '</div>'
                    + '</div>';
                document.getElementById("heatingSys").innerHTML = text;
            }
        }
    })
