fetch("./assets/json/product-list.json")
    ...
    .then(data => {
        ...
        let jsonObject = data.responses[0][0].params.recommendedProducts["Bilgisayar, Tablet > Dizüstü Bilgisayar (Laptop)"];
        let text = "";
        ...
        for (const key in jsonObject) {
            if (Object.hasOwnProperty.call(jsonObject, key)) {
                ...
                text += '</div>'
                    ...
                    + '</div>'
                    + '</div>';
                document.getElementById("laptop").innerHTML = text;
            }
        }
    })
